...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-xl font-bold text-gray-900 leading-tight">Formulir Tambah Guru/Staf</h2>
    </div>
</div>

<div class="bg-white border border-gray-200 shadow-sm rounded-2xl overflow-hidden max-w-4xl">
    <form action="{{ route('admin.teachers.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8">
        @csrf
        
        <div class="space-y-6">
            
            <div x-data="imagePreview()" class="flex flex-col sm:flex-row gap-6 items-start">
                <div class="w-full sm:w-1/3">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Foto Profil</label>
                    <div x-show="!imageUrl" class="flex flex-col items-center justify-center p-6 border-2 border-gray-300 border-dashed rounded-xl bg-gray-50 hover:bg-white transition cursor-pointer" @click="$refs.fileInput.click()">
                        ...
                        <span class="text-sm font-medium text-primary-600 text-center">Pilih Foto</span>
                        <p class="text-[10px] text-gray-500 mt-1 text-center">Rasio 3:4 maks 2MB</p>
                    </div>

                    <div x-show="imageUrl" class="relative rounded-xl overflow-hidden border border-gray-200 shadow-sm aspect-[3/4] max-w-[200px] mx-auto sm:mx-0" style="display: none;">
                        <img :src="imageUrl" class="w-full h-full object-cover" alt="Preview Image">
                        <button type="button" @click="removeImage" class="absolute top-2 right-2 bg-red-600/90 text-white rounded-md p-1.5 hover:bg-red-700 transition focus:outline-none">
                    ...
                    @error('photo')
                        <p class="mt-1.5 text-sm text-red-600 font-medium pb-2 text-center sm:text-left">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex-1 w-full space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="position" id="position" value="{{ old('position') }}" required list="position_options"
                        ...
                        <div>
                            ...
                            <input type="text" name="subject" id="subject" value="{{ old('subject') }}" list="subject_options"
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-[11px] text-gray-500">Angka terkecil (0) akan tampil paling awal. Berguna untuk mengurutkan Kepala Sekolah di atas.</p>
                        </div>
                        <div>
