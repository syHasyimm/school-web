...
@php
    $classes = match($type) {
        ...
        'error', 'danger' => 'bg-red-50 border-red-400 text-red-800',
        'warning' => 'bg-amber-50 border-amber-400 text-amber-800',
        default => 'bg-sky-50 border-sky-400 text-sky-800',
    };
...
<div
    ...
    {{ $attributes->merge(['class' => "flex items-start gap-3 px-4 py-3 rounded-xl border {$classes}"]) }}
    role="alert"
>
    {{-- Icon --}}
    ...
    {{-- Dismiss --}}
    @if($dismissible)
        <button @click="visible = false" class="shrink-0 opacity-60 hover:opacity-100 transition-opacity cursor-pointer">
