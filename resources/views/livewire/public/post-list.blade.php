        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
                ...
                <div class="w-full md:w-1/3 relative">
                    <div class="relative">
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari artikel..." class="w-full pl-10 pr-4 py-3 rounded-xl border-gray-200 focus:border-primary-500 focus:ring-primary-500 shadow-sm transition-all text-sm">
                ...
                <div class="flex flex-wrap gap-2 w-full md:w-auto h-full items-center">
                    <button wire:click="setCategory('')" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ !$category ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200' }}">Semua</button>
                    @foreach($categories as $cat)
                        <button wire:click="setCategory('{{ $cat->slug }}')" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $category == $cat->slug ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200' }}">{{ $cat->name }}</button>
                    @endforeach
                </div>
            </div>
            ...
            <div wire:loading class="w-full text-center py-10">
                ...
                <p class="text-sm mt-3 text-gray-500 font-medium">Memuat data...</p>
            </div>
